...
def download_and_convert_to_pandas(
    ...
    with ConfiguredBoto3Session(
        ...
        sqlite_file_path = _download_chunk_to_temp_file(s3_client, chunk_name)
        ...
        if overflow_chunks:
            for overflow_chunk in range(1, overflow_chunks + 1):
                logger.debug(
                    f"downloading overflow chunk "
                    f"{f'{chunk_name}b{overflow_chunk}'}"
                )

                overflow_chunk_file_path = _download_chunk_to_temp_file(
...
def convert_to_df(
    ...
    try:
        ...
        overflow = read_metadata_from_sqlite(file_path)
...
def _download_chunk_to_temp_file(
    client: ConfiguredBoto3Session,
    chunk_name: str,
) -> Optional[Path]:
